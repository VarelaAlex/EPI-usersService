...
routerClassrooms.post("/", authenticateToken, isTeacher, async (req, res) => {
	...
	let classroom = null;
	try {
		let classroomName = await database.query(
			"SELECT name FROM classrooms WHERE name = ? AND teacherId = ?",
			[name, teacherId]
		);
		...
		classroom =
			await database.query(
				"INSERT INTO classrooms (name,level, teacherId) VALUES (?,?,?)",
...
routerClassrooms.get("/list", authenticateToken, isTeacher, async (req, res) => {
	...
	try {
		classrooms =
			await database.query(
				"SELECT c.id, c.name, COUNT(s.id) numberStudents FROM classrooms c LEFT JOIN students s ON s.classroomId=c.id WHERE teacherId=? GROUP BY c.name",
				[teacherId]
			);
	}
	catch ( e ) {
...
routerClassrooms.put("/:classroomName", authenticateToken, isTeacher, async (req, res) => {
	...
	let classroom = null;
	try {
		let classroomNameFound = await database.query(
			"SELECT name FROM classrooms WHERE name = ? AND teacherId = ?",
			[name, teacherId]
		);

		if ( classroomNameFound.length && classroomName !== name ) {
			return res.status(404).json({ error: { repeatedName: "classrooms.create.error.repeated" } });
		}

		classroom =
			await database.query(
				"UPDATE classrooms SET name = ?, level = ? WHERE name = ? AND teacherId = ?",
				[name, level, classroomName, teacherId]
			);
	}
	catch ( e ) {
		return res.status(500).json({ error: { type: "internalServerError", message: e } });
...
routerClassrooms.delete("/:classroomName", authenticateToken, isTeacher, async (req, res) => {
	...
	try {
		result =
			await database.query("DELETE FROM classrooms WHERE name = ? AND teacherId = ?", [classroomName, teacherId]);
	}
	catch ( e ) {
		return res.status(500).json({ error: { type: "internalServerError", message: e } });
	}
